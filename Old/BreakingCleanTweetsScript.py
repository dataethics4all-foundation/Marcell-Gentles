...
def vectorize(cleanedData, vocab= None, makeCSV= False, model= None):
    if model==None:
        ...
        if model[1] == 'BoW':
            modelTransform = model.transform(cleanedData["Cleaned"])
            modelVocab = model.vocabulary_
        ...
        column_list = []
        for word in modelVocab:
            column_list.append("M: " + word)
        
        model_dense_df = pd.DataFrame(modelTransform.todense(), columns = column_list)
        model_dense_df.reset_index(inplace=True)
    ...
    vocab = {'Model': modelVocab}
    
    return cleanedData, vocab

print(vectorize(cleanDF("training.1600000.processed.noemoticon.csv", 5)))
